...
## 손으로 그린 벡터 — Higgsfield를 안 쓰고 채운 11장 (2026-08-28)

구아슈 화풍을 34구절 전부로 넓힐 때 크레딧이 떨어지고 검수에서 걸린 자리가
남았다(3·4b·7c·10c·11c·12b·13b·16c·32b·33c — 3번은 두 시안 다 걸려 그림이
아예 없었다). 「Higgsfield 없이 직접 만들어 줄 수 있나」는 요청으로, 이
자리들만 **손으로 SVG를 짜고 헤드리스 크롬으로 캡처해 WebP로 바꿨다**(비용
0, API 호출 없음). 이 방식으로 만든 11장은 34장 본편(수채·먹선)이나 구아슈
짝과 **화풍이 다르다** — 붓 질감·번짐 없이 먹선+단색 면으로 된 평평한
일러스트다. 파일은 위 34장·구아슈와 똑같이 `img/verse/<번호><b|c>.webp`.

**만드는 법**: `<svg viewBox="0 0 1080 806">`를 채운 HTML 파일을 스크래치
패드에 쓰고, 헤드리스 크롬으로 `--window-size=1080,806 --screenshot=`을
찍어 PNG로 받은 뒤 Pillow로 WebP 변환. AI 생성 규칙(사람 없음·글자 없음
등)은 애초에 내가 그리므로 자동으로 지켜진다 — 대신 **직접 본 뒤 고치는
과정이 그만큼 더 든다**(아래 겪은 것 참고).

**공통 화풍**: 크림 배경(라디얼 그라데이션 `#fffdf6→#f6efdb`), 먹선
`#4a3423`(굵기 3~5px, 둥근 선끝·이음), 온기 있는 팔레트(금·주황·갈색을
중심으로, 장면에 맞춰 보라·초록·달빛 남색을 더함). `style.css`의
`--gold`(#c8a84b)·`--cream`(#fdf8f0)과 같은 계열로 골랐다.

⚠️ **겪은 것 — 손으로 그릴 때도 실패가 있다**:
- **동물은 어렵다**: 13번(나귀)을 매끄러운 윤곽선 하나로 그리려다 두 번
  실패했다(관절이 어색하거나 뭉친 구름처럼 나옴). **몸통·목·머리를 각각
  도형으로 만들고 서로 넉넉히 겹치게 배치**해서야 됐다 — 목은 회전시킨
  둥근 사각형(`transform: translate(...) rotate(...)`)으로 몸통과 머리
  둘 다에 파고들게 걸쳤다.
- **물체가 배경에 반쯤 파묻히면 이상해진다**: 10번(동전) 첫 시안은 원을
  탁자 판 중앙에 그려 반은 판 위, 반은 판 뒤로 나와 "박힌" 것처럼
  보였다. 탁자를 먼저 그리고 **동전을 그 위에 완전히 얹은 납작한
  타원**으로 바꾸니 해결됐다.
- **애매한 낙서는 글자로 오해살 수 있다**: 3번(두루마리) 리본을 곡선
  하나로 대충 그렸더니 화살표나 체크 표시처럼 보였다. 매듭 모양을
  또렷한 삼각형으로 다시 그렸다.
- 이 셋 다 AI가 만든 실수가 아니라 **내가 만든 실수** — 그래서 검수를
  "사람 눈으로 본다"가 아니라 "내가 그리고 내가 본다"로 두 번 거쳤다.

**34장 본편에는 반영하지 않는다** — 이 11장은 구아슈·수채 짝과 나란히
순환되는 "세 번째(또는 두 번째) 그림"일 뿐, 34장 전체를 벡터로 바꾸는
게 아니다. 전체를 벡터로 통일하고 싶으면 91장(수채+구아슈)을 버리고
34구절을 다시 그려야 한다 — 지금은 하지 않는다.
