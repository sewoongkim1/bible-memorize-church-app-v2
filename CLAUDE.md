...
- **어려운 도전(2026-08-25):** 다섯 번에 한 번, 도전 화면의 **칸 너비를 모두 같게** 한다(`HARD_EVERY`·`hard-challenge-count`). 지우는 것은 밑줄이 아니라 **너비**다 — `width`가 단어 길이를 따라가서 「사랑하라」와 「내」가 모양만으로 구분됐고, 그게 가장 큰 힌트였다. 밑줄·칸 개수는 그대로 둔다(어디에 넣는지까지 감추면 길만 잃는다). 어려운 차례에는 **3단계까지 마친 구절에서 고른다**(`hardPool`) — 익히는 중인 구절에 글자 수까지 감추면 벽이 된다. ⚠️ **예고와 실제가 같은 함수(`hardNext`)를 본다**: 처음엔 '뽑은 뒤에 자격을 보고 내린다'로 짰는데 그러면 완료 화면이 매번 「다음은 어려운 도전」이라 해 놓고 오지 않을 수 있었다. **「자동으로 계속 도전」은 완료 화면을 건너뛰어 예고를 볼 자리가 없으므로**(`renderChallengeDone` 첫 줄) 도전 화면 자체를 달라 보이게 했다 — 배지를 금색 바탕에서 남색 바탕·금색 글씨로 뒤집었다. 「이번엔 그냥 할래요」로 접으면 세지 않아 다음 도전에 다시 온다(없애는 게 아니라 미루는 것). 설계: `docs/superpowers/specs/2026-08-25-hard-challenge-design.md`
