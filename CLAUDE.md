...
- [ ] 필사 신청 알림 실동작 확인: 담당자 등록은 2026-08-25에 마쳤다(`app_config.pilsaAdmins`). 본인 제외가 없으므로 **담당자가 직접 신청해 보면 자기 폰으로 온다** — 「[필사 신청]」(새 건)·「[필사 신청 변경]」(고친 건).
  - ⚠️ 알림 결과가 어디에도 남지 않는다: `pilsaApply`가 `pilsaNotifyAdmins`를 `try/catch`로 감싸 버리고, 담당자가 없으면(`no-admin`) `push_log`에도 안 남는다. 등록이 틀어져 아무에게도 안 가도 조용하다 — 필요하면 이 자리를 `push_log`에 남기게 고칠 것
