...
- [x] **iOS 2단계(네이티브 기능 3종) — 위젯·네이티브 로그인 화면 실기기 확인 완료(2026-09-16).**
      ...
      **3단계(App Store 정식 심사 제출) 준비물도 완료(2026-09-16)** — `store/listing-ios-ko.txt`
      (설명·키워드·App Review 로그인 안내, 네이티브 기능 명시) · `store/screenshots-ios/*.png`
      (6.7인치 8장, `tools/capture-ios-shots.py`로 재생성 가능) · `store/README.md`의
      "App Store(iOS) 등록 준비물" 절(App Privacy 세부정보 매핑·연령 등급·수출 규정 전부 포함).
      아이패드는 실기기 미확인이라 지원 범위에서 뺐다(`TARGETED_DEVICE_FAMILY`를 아이폰 전용 1로).
      푸시는 마케팅 문구·심사 메모에서 일부러 안 내세웠다(위젯·로그인만으로 4.2 대응 근거 충분).
      **다음**: App Store Connect에서 이 준비물로 실제 심사 제출(사람이 직접) → 반려 시 사유 보고
      → Mac 생기면 콘솔 로그로 푸시 원인도 마저 확인.
