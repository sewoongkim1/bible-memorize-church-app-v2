...
- [ ] **사역신청(2026) — 담당자 데이터 확정 대기(개발 착수 전)**: 종이 「사역신청서」를
      ...
      **개발보다 데이터 확정이 먼저**다. 그 확인용 엑셀 양식(`tools/ministry-form-gen.py`
      → `ministry/2026_사역신청_부서확인양식.xlsx`, 초안 98팀)은 이미 있다 — 부서
      확인이 끝나면 그 표기명이 `ministry_catalog` 최종 시드가 된다. 필사 노트 신청과
      같은 뼈대(신청/취소/상태전환+Web Push)를 재사용할 계획.
      ...
      **2026-09-07 — 앱이 열리기 전까지는 올해도 종이로 받는다**(성도님 확정:
      ...
      바뀌면 다시 돌리기만 하면 된다. 3단 체크리스트 + 임명직 별도 상자로
      01장이 지적한 "임명직이 표에 섞여 안 보인다" 문제를 없앴다. ⚠️ **아직 초안
      이름 그대로**라 부서 확인 전엔 인쇄해 나눠주지 말 것.
