...
- **오늘의 찬양(2026-09-23 개시)** — 서버가 우리 교회 찬양 **하루 한 곡**을 정하고(모두 같은 곡), **매일 묵상 창 맨 아래**의 `🎵 찬양 · 곡명` 을 누르면 **앱 안 찬양 화면**(`renderSongScreen`·파사드)에서 재생된다. ⚠️ 손대기 전에 `docs/notes/today-song.md` — **개시한 날 하루에 두 번 뒤집힌 자리**(찬양 앱으로 보냈다가 성도님 제보로 다시 앱 안 재생 · 옮기기 전에 그 절을 다 읽을 것)·「기록을 비우고 새 바퀴」 금지·LRU 를 곡별 `max(day)` 로 묶는 이유·**유튜브 임베드 규칙 여덟**(파사드·nocookie·`fs=0`·PiP 금지·`iframe_api` 를 탭 뒤에만·나갈 때 세 단계 등)·게이트 키를 `PUBLIC_CONFIG_KEYS` 에 넣어야 하는 것이 있다. **임베드라 개인정보 방침을 고쳤다**(`privacy/` 2·6항 · 앱 안 화면 둘 · 스토어 신고는 담당자 확인 목록).
...
- [ ] 🤖 **안드로이드 위젯 — 2026-09-21 시험판 실기기 확인 끝(갤럭시 S23 울트라). 남은 것: 비행기 모드 · 다음 날 아침 확인, 그리고 9/27 프로덕션 승인 뒤 계획 Task 8(서명·판 번호·업로드).**
      ...
      **유리한 점 셋:** ① 서버는 이미 끝났다(`getWeeklyVerse`·`getTodayMeditation`·`getTodayBlessing`
      을 그대로 쓴다) ② 맥이 필요 없다(이 PC 에서 Gradle 빌드) ③ TWA 는 도메인과 연결돼 있어
      위젯에 `gocheok.onlybible.kr/?v=38` 같은 주소만 넣으면 앱이 열린다(아이폰처럼 별도 스킴이 필요 없다).
      **할 일:** Bubblewrap 으로 프로젝트 생성 → 저장소에 넣기 → AppWidgetProvider + 갱신(WorkManager)
