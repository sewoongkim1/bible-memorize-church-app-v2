...
- [ ] ⏰ **시편 말씀 액자 — 2026-09-21(월) 1일차 열림, 남은 날이 적다.**
      기획 확정본은 `docs/superpowers/specs/2026-09-10-psalm-frame-design.html`(HTML, 14장).
      시편 180구절을 하루 한 편씩 열어 액자 안에서 0~3단계로 외우는 새 코너.
      **임계경로는 개발이 아니라 자료다** — 담당자의 180구절 개역개정 본문(마감 09-17 목)
      없이는 줄수 측정도 시드도 시작되지 않는다. **막히면 두 갈래로 푼다:**
      ① 시작일은 `app_config('psalm')`의 `start` 한 줄이라 **배포 없이** 미룰 수 있다
      ② 하루 한 편씩만 쓰이므로 **첫 30편만 있어도 한 달을 간다**(뒤이어 채워도 안 깨진다).
      ⚠️ **5줄 규칙은 3단계(전체 빈칸) 기준으로 잰다** — 빈칸이 단어마다 1em씩 넓어져
      0단계 기준으로 맞추면 3단계에서 6~7줄이 된다. 잣대는 `글자수 + 단어수 ≤ 76`.
      ⚠️ **`challenge_log.mode`를 새로 만들지 않는다** — 시편은 `verse_no ≥ 1001`로 이미
      갈리므로 `learn-typing` 그대로 쓴다(CHECK 제약 사고를 원천 회피).
      ⚠️ **`app.js`의 복습 큐(6867줄)가 유일한 필수 수정** — 지금은 `verses`만 훑어
      시편 구절이 큐에서 조용히 사라진다. 다음은 구현 계획서.
