...
- [ ] **도전 전환율** — 질의를 `supabase/challenge_funnel.sql`에 **파일로 고정**했다(다시 잴 때 손으로 다시 쓰면 잣대가 달라져 견줄 수 없다). 2026-08-25 기록은 오늘 33명 중 8명(24%)·누적 133명 중 31명(23%)인데 그때 쓴 질의가 남아 있지 않아 같은 잣대인지 확인할 길이 없다. **2026-09-02 첫 화면 개편으로 깔때기 자체가 바뀌었으므로** ①②③을 지금 돌려 기준선을 이 줄에 적어 둘 것. ④(새로 오신 분 코호트)는 **2026-09-09 이후**에 봐야 뜻이 생긴다
