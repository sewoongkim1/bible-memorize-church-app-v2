...
- **가정 축복 기도문(2026-09-03, v3.273):** 104편을 **「오늘 한 편 + 주제로 찾기」 한 화면**에 담았다(`renderPrayerBook`). 자료는 `blessings` 표 → `getBlessings`.
  ...
  - **성도님 첫 화면에는 아직 없다** — `admin.html`의 「가정 축복 기도문 (준비 중)」 또는 `index.html?preview=prayer`로만 열린다. 열 때는 `renderSummary`의 「함께」 묶음에 한 줄을 더하면 된다.
