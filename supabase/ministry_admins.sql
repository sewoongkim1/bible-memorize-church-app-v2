...
-- ⚠️ 담당자의 이름·소속이 바뀌면(성도 정보 관리) identity_key 가 바뀌어 **로그인이 막힙니다**.
--    pilsaAdmins 는 member_profile · member_merge 가 따라 고치지만 ministryAdmins 는 아직 아닙니다
--    — 그때는 이 SQL 을 새 소속으로 다시 돌리세요. (막히는 쪽으로 틀리므로 새어 나가지는 않습니다.)
--
-- ── 쓰는 법 ─────────────────────────────────────────────────
