...
-- ⚠️ **개발 DB 도 여러 세션이 함께 쓴다**(2026-09-23 실제로 겪었다 — 이 시드를 넣는 동안
--    다른 세션이 users 를 1명에서 32명으로 늘리고 challenge_log 를 갈아 끼웠다).
--    그래서 5-a 의 log_rows 나 5-d 의 사람 수 같은 **총계는 기대와 안 맞을 수 있다.**
--    내 시드가 제대로 들어갔는지는 **목장 번호로 내 것만 따로 세어** 본다:
select (select count(*) from public.users where mok in ('90','91','92','93','94','95','96','99'))
         as 내시드_사람,                                    -- 기대 8
       (select count(*) from public.challenge_log c join public.users u on u.id = c.user_id
         where u.mok in ('90','91','92','93','94','95','96','99'))
         as 내시드_로그;                                    -- 기대 177
--   ⚠️ 5-b(순위 여덟 줄)와 5-c(믿음 5 · 사랑 3)는 **남의 데이터가 섞여도 그대로여야 한다** —
--      다른 세션 사람들은 목장 번호가 달라 같은 소속으로 안 묶이기 때문이다.
--      만약 5-c 에 모르는 소속이 3명 이상으로 뜨면 그건 남의 시드다. 지우지 말 것.
