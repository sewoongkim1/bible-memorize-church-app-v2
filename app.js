...
`;
  ...
  fillWeeklySummaryBtn(weeklyVerse); // 설교요약이 있으면 '설교보기' 옆에 버튼 추가
...
// 이번주 말씀 카드에서도 설교요약을 열 수 있게 — 뒤로는 요약 화면으로
function fillWeeklySummaryBtn(verse) {
  ...
  loadSermons().then((sermons) => {
    ...
    if (!s || !el) return;
    el.outerHTML = `<button class="weekly-secondary" id="weekly-summary">설교요약</button>`;
