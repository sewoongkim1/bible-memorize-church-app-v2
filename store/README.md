...
| `screenshots-ios/*.png` | **App Store Connect → 미디어 관리 → 6.7인치 디스플레이** (1290×2796, 8장) |

스크린샷은 `python tools/capture-ios-shots.py`로 다시 뽑을 수 있다(운영 DB 안 건드림 —
`tools/capture-guide-shots.py`와 같은 원리로 fetch를 가로채고 가짜 이름 "홍길동"만 쓴다).
⚠️ **화면을 고쳤으면 다시 뽑는다** — 안드로이드 스크린샷과 같은 함정이다.

⚠️ **6.9인치(1320×2868, iPhone 16 Pro Max급)는 아직 안 뽑았다.** App Store Connect가
"권장"으로 요구하는 크기인데, 6.7인치만 있어도 나머지 화면 크기는 자동으로 맞춰 준다
(2026-09 기준). 나중에 심사에서 요구하면 `tools/capture-ios-shots.py`의 `W, H` 값을
1320, 2868로 바꿔 다시 뽑으면 된다.
...
### 3. 스크린샷 업로드
**미디어 관리 → 6.7인치 디스플레이**에 `screenshots-ios/` 8장을 순서대로 올린다
(처음 2~3장만 목록에서 먼저 보이니, 안드로이드와 같은 순서 — `home → stage3 → list` 순을 권장).
