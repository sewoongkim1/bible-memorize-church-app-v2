...
9자리)가 필요하다 — 없으면 무료로 새로 신청(`developer.apple.com/enroll/duns-lookup` 또는
`dnb.co.kr`, 보통 5영업일).
...
협조·등록비 승인)과 담당자에게 보낼 안내문은 `church-org-registration.pdf`
(원본 `church-org-registration.html`)로 만들어 뒀다 — 다시 설명할 필요 없이 그대로 보내면 된다.
