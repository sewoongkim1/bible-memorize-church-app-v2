...
알림 동의 문구가 **「아침만」이라고 일곱 곳에서 약속**하고 있다. 저녁 알림이 나가는 순간 일곱 다 거짓이 된다.
(설계를 적을 때는 넷으로 셌는데, 실행 계획을 짜며 다시 훑어 **사용 설명서 두 줄과 알림 권유 카드**를 더 찾았다.)
...
| app.js 의 알림 권유 카드 `.pn-title` | 「🔔 **매일 아침**, 오늘의 묵상을 받아보세요」 |
| app.js 의 알림 권유 카드 `.pn-sub` | 「하루 한 구절 · 짧은 묵상으로 **하루를 시작해요**」 |
| app.js 사용 설명서 「알림 켜기」의 `lead` | 「**아침에** 오늘의 말씀을 알려 드려요.」 |
| app.js 사용 설명서 「알림 켜기」의 `steps` | 「알림 받을 시간(**아침 5~8시**)은 ⚙️ 설정에서 …」 |

이 가운데 `privacy/index.html` 은 **로그인 없이 열리는 공개 방침**이고 플레이스토어 심사에 낸 문서다.
...
**서버** — `updatePushEvening` 액션(`user_id` + `on`) 하나. 이름을 `updateIosPushHour` 와 맞췄다 —
프런트의 `setPushEvening` 과 헷갈리지 않게. `savePush`·`saveIosPushToken` 은 건드리지 않는다
(기본값이 칸에서 나온다). 이 액션이 **두 표를 함께** 고친다.

**프런트**
- `js/push.js` 에 `getPushEvening()` / `setPushEvening(on)` — `setPushHour` 와 같은 꼴.
  ⚠️ **네이티브 분기를 「두는」 것이 아니라 「필요 없게」 만든다.** `setPushHour` 는 웹푸시
  구독(endpoint)이 있어야 해서 분기가 필요했고, 그 분기를 빠뜨려 2026-09-16 에 아이폰에서
  조용히 무시됐다. 저녁은 **사람 단위**(`user_id` 하나)라 서버가 두 표를 함께 고치면 되고,
  그래서 분기 자체가 없다. 회귀 시험 `tests/push-evening-native.py` 가 그 자리를 지킨다.
- ⚠️ `localStorage` 를 못 읽어도 **켜짐**으로 본다 — 읽기 실패가 성도님을 조용히 끄면 안 된다.
- 설정 화면에 토글 한 줄. 글은 「🌙 저녁 8시에도 받기」.
- 위 문구 일곱 곳 정정 — 실제 목록과 바꿀 글은 실행 계획 `docs/superpowers/plans/2026-09-23-evening-push-stage0.md` Task 4.
