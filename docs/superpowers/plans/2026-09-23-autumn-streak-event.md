...
`supabase/event_streak_metrics.sql`:

```sql
-- 가을 말씀 동행 — 기준선과 효과 측정
-- ⚠️ 이 저장소는 공개(public)입니다 — 비밀번호·키를 절대 넣지 마세요.
--
-- 쓰는 법: 아래 ① 한 줄(창 시작일)만 바꿔 가며 같은 질의를 돌린다.
...
-- ⚠️ mode 를 세지 않는다. 그날 daily_activity 에 행이 있으면 한 칸이다(카드가 65.2%).

-- ① 창 ─────────────────────────────────────────────────────────
--   A 직전 6주   2026-08-31 ~ 2026-10-11   (이벤트 「말씀암송이 답이다!」 꼬리가 섞여 있다)
--   B 평시 6주   2026-07-06 ~ 2026-08-15   (이벤트 전)
--   C 본 회차    2026-10-11 ~ 2026-11-21
-- 아래 :start 자리에 그 날짜를 넣고 돌린다.

-- ② 문턱별 인원 ────────────────────────────────────────────────
with p as (select date '2026-08-31' as s, 6 as weeks, 3 as per_week),
d as (
...
-- ⑤ 종료 후 4주 지속 (12월에 돌린다) — 위 ② 를 s = '2026-11-22' 로 다시.

-- ─────────────────────────────────────────────────────────────
-- 결과란 — 돌린 날과 값을 여기에 적어 둔다. 적지 않으면 다시 잴 때 견줄 것이 없다.
--
-- 2026-09-23 (공지 전) · 창 2026-08-09 ~ 09-19
--   활동 185 · 1주 83 · 2주 58 · 3주 40 · 4주 16 · 6주 6
--   ⚠️ 이 창은 「말씀암송이 답이다!」(8/16~9/30) 한복판이라 낙관적이다.
--
-- (A 2026-08-31~10-11) 돌린 날:            결과:
-- (B 2026-07-06~08-15) 돌린 날:            결과:
-- 공지 나간 날:
```

- [ ] **Step 2: A 창을 돌린다**

운영 SQL Editor 에 **② 블록만** 붙여넣고 RUN 하는 것이 가장 간단하다.
명령줄로 하려면:

```bash
SP="$(mktemp -d)" && mkdir -p "$SP/supabase" && cp -r supabase/.temp "$SP/supabase/"
supabase --workdir "$SP" db query --linked "$(cat <<'SQL'
<여기에 ② 블록을 그대로 붙여넣는다 — 날짜만 바꿔서>
SQL
)"
rm -rf "$SP"
```

기대: `활동한 분` 등 여섯 숫자가 담긴 행 하나.
⚠️ 저장소 루트 `.env` 의 키 이름에 하이픈이 있어 CLI 가 dotenv 파서에서 죽는다 —
   위처럼 **`.env` 가 없는 다른 폴더**에서 부른다. ⚠️ 다 쓰면 지운다(`linked-project.json` 이 들어 있다).
⚠️ 이것은 **운영을 읽는다.** SELECT 뿐이라 안전하지만 쓰기 질의를 섞지 않는다.

- [ ] **Step 3: B 창을 돌린다**

`with p as (select date '2026-07-06' …` 로 날짜만 바꿔 같은 명령을 돌린다.

- [ ] **Step 4: 결과란을 채운다**

Step 2·3 의 숫자와 돌린 날짜를 파일 맨 아래 결과란에 적는다. **빈칸으로 두지 않는다.**

- [ ] **Step 5: 커밋**

```bash
git commit -m "$(printf 'feat(이벤트): 가을 말씀 동행 기준선 질의 — 공지 전에 떠 둔다\n\n공지가 나가면 숫자가 움직인다(지난 이벤트에서 공지 주부터 전환율 14.7→27.0%%).\nA 직전 6주 · B 평시 6주 두 창을 같은 질의로 떠서 결과란에 박았다.\n\nCo-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>')" -- supabase/event_streak_metrics.sql
```
