...
```bash
git diff --cached --name-only
git commit -- index.html app.js js/psalm.js js/api.js sw.js guide/index.html -m "chore: 열람 기록 배포 — 판 번호"
git push
```

⚠️ `git add -A` 금지. `bump.py` 가 건드리는 것은 `index.html` 과 `app.js` 뿐이다.
