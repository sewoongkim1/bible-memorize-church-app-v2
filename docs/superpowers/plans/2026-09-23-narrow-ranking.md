...
## 진행 상황 (2026-09-23 · 세션 종료 시점)

| Task | 상태 | 커밋 |
|---|---|---|
| 1 순수 함수 · 검사 · preflight | ✅ 끝 | `ea9a372` |
| 2 개발 DB 시드 | ✅ 끝 · **개발 DB에 실제로 반영함** | `b074df3` · `6087944` |
| 3 받아오기/그리기 분리 | ✅ 끝 | `74e1e0b` |
| 4 칩 · 좁힌 목록 · CSS | ✅ 끝 | `cb3fbd3` |
| 5 측정 한 줄 | ✅ 끝(⚠️ 아래) | `20c878a` |
| 6 문서 | ✅ 노트·CLAUDE.md | `28ebdb0` |
| 6 설명서 한 줄 | ⏸ **app.js 작업 트리에 미커밋** | — |
| 6 bump · 푸시 · 배포 확인 | ⏸ **조율 대기** | — |

### 남은 셋

1. **app.js 설명서 한 줄이 커밋 안 됐다.** `MANUAL` 의 순위 절에 「**우리 교구**를 누르면 …」 한 줄이
   작업 트리에만 있다. 다른 세션이 같은 파일을 동시에 고치고 있어 **경로 커밋을 못 썼다** —
   `git commit -- app.js` 는 **작업 트리를 다시 스테이징**하므로 남의 헝크가 딸려 간다.
   app.js 가 깨끗해지면 경로 커밋으로, 아니면 임시 인덱스(`GIT_INDEX_FILE`)로 담는다.
2. **판 올리기·푸시.** 2026-09-23 기준 **미푸시 커밋 69개**가 쌓여 있다(여러 세션 것). 이 저장소는
   **푸시가 곧 배포**라, 무엇이 성도님께 나가도 되는지 각 세션과 순서를 맞춘 뒤 **bump 는 한 번만** 돌린다.
3. **운영 Edge Function 배포.** `FEATURES` 의 `ranking-scope` 는 커밋됐지만 운영에 안 나갔다.
   다른 세션의 `docs/superpowers/plans/2026-09-23-feature-log.md` **Task 9** 와 함께 한다.
   ⚠️ 운영에 `feature_log` 표도 아직 없다 — 그전까지 기록은 0이고 그건 정상이다.

### ⚠️ Task 5 커밋에 남의 코드가 섞였다

`20c878a` 에 다른 세션의 이벤트 코드 **105줄**이 함께 들어갔다.
`git apply --cached` 로 내 헝크만 담았는데 **`git commit -- <경로>` 가 그것을 무시하고 작업 트리를
다시 스테이징**했기 때문이다(메모리 `stage-only-changed-files` ③이 적어 둔 기전).
**되돌리지 않았다** — 그 사이 다른 세션이 커밋해 HEAD 가 옮겨갔고, 저장소 방침이
「남의 파일이 내 커밋에 섞였으면 되돌리지 말고 알린다」이다. 작업 손실은 없다.

### 다음에 이 화면을 찍을 사람에게

`tools/screen-sweep.py` 로 순위 화면을 찍으면 **묵상 창이 칩 줄을 가린다.** 홈에서 제거해도
타이머로 다시 뜨므로 제거 JS 로는 못 막는다. 스크래치에 복사본을 만들어 스텝에
「모달 제거 + 측정값을 화면 맨 위 흰 상자에 찍기」를 넣어 **숫자로 쟀다**
(`docs/notes/capture-tools.md` 의 「사진 대신 숫자로 재라」와 같은 방법).
