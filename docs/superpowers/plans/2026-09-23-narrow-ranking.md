...
| 6 설명서 한 줄 | ✅ 끝 | `75945b6` (app.js 1줄) |
| 6 bump · 푸시 · 배포 확인 | ⏸ **조율 대기** | — |

### 남은 둘

1. **판 올리기·푸시.** 2026-09-23 기준 **미푸시 커밋 69개**가 쌓여 있다(여러 세션 것). 이 저장소는
   **푸시가 곧 배포**라, 무엇이 성도님께 나가도 되는지 각 세션과 순서를 맞춘 뒤 **bump 는 한 번만** 돌린다.
2. **운영 Edge Function 배포.** `FEATURES` 의 `ranking-scope` 는 커밋됐지만 운영에 안 나갔다.
